...
/**
 * Everything the single-week action list needs. Reads only — all recording
 * goes through previewAllocation/recordPayment (2.19: one engine).
 */
export async function getWeekBoard(input?: { weekNumber?: number }) {
  ...
  try {
    ...
    if (cycle.weeks.length === 0) {
      return { ok: false as const, error: "This cycle has no weeks yet." };
    }
    const targetNumber = resolveTargetWeek({
      requested: input?.weekNumber,
      cycleWeek,
      weekNumbers: cycle.weeks.map((w) => w.weekNumber),
    });
    const week = cycle.weeks.find((w) => w.weekNumber === targetNumber)!;
    ...
    for (const participation of cycle.participations) {
      const finishWeek = calculateFinishWeek(participation.startWeek, participation.weeksCommitted);
      const inWindow =
        participation.startWeek <= week.weekNumber && week.weekNumber <= finishWeek;
      if (!inWindow) continue;

      const { standing } = standingFor(cycle, participation, cycleWeek, today);
      const row = participation.payments.find((p) => p.weekId === week.id) ?? null;
      const isDeferred = (row?.isDeferred ?? false) || week.isSkipped;
      const amountPaidThisWeek = row?.amountPaid ?? 0;
      if (!isDeferred) expected += participation.weeklyAmount;
      receivedTotal += amountPaidThisWeek;
      ...
      members.push({
        ...
        amountOwed: standing.amountOutstanding,
      });
    }

    // Receipts that landed on this week, so each paid row can be undone
    // (deleting the receipt rebuilds the week aggregates).
    const allocations = await prisma.paymentAllocation.findMany({
      where: { payment: { weekId: week.id } },
      include: {
        event: { select: { id: true, amount: true, method: true, receivedAt: true } },
        payment: { select: { participationId: true } },
      },
      orderBy: { event: { receivedAt: "asc" } },
    });
    const receiptsByParticipation: Record<
      string,
      { eventId: string; appliedHere: number; eventAmount: number; method: string | null }[]
    > = {};
    for (const a of allocations) {
      (receiptsByParticipation[a.payment.participationId] ??= []).push({
        eventId: a.event.id,
    ...
    const { owing, paid } = splitWeekRoster(members);
    const daysLeft = Math.floor(
      (utcDay(week.date) + PAYMENT_WINDOW_DAYS * MS_PER_DAY - utcDay(today)) / MS_PER_DAY,
    );
    ...
    const full = {
      ...
      allWeeks: cycle.weeks.map((w) => ({ weekNumber: w.weekNumber, date: w.date })),
      expected,
      receivedTotal,
      membersPaid: paid.filter((m) => !m.isDeferred).length,
      membersExpected: members.filter((m) => !m.isDeferred).length,
      windowDaysLeft: daysLeft,
      owing,
      paid,
      receiptsByParticipation,
    };

    // Presentation mode (2.4): identities become lucky numbers, amounts and
    // receipts are not sent.
    if (await getSetting("presentationMode")) {
      const labels = new Map(
        cycle.participations.map((p) => [
          p.id,
          numbersLabel(p.luckyNumbers.map((n) => n.number)),
        ]),
      );
      return {
        ok: true as const,
        data: redactWeekBoard(full, (id) => labels.get(id) ?? "Member"),
      };
    }
    return { ok: true as const, data: full };
  } catch (e) {
    console.error("getWeekBoard failed:", e);
    return { ok: false as const, error: `Could not load the week. ${errorMessage(e)}` };
  }
}
